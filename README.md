...
- **Opcional**: formulario de reserva con fechas de entrada y salida. Usa `useActionState` de React con una server function `POST` que recibe el `FormData`, lo valida y calcula el precio en el servidor. A diferencia de Next, no hace falta `"use client"`: en TanStack Start los componentes son isomórficos y la frontera cliente/servidor está en las funciones (`createServerFn`), no en los componentes.